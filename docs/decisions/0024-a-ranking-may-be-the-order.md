...
Two more documents, in domains chosen because they attack what was still fragile: a
...
is not a supposition: the document was built, and it produces three `OR-303` errors on a
...
categories — properties of the category. §9.1.1 closed the member list, so those numbers went
into `description` in both, where §6.1.6 promises nothing will read them.

**The ties table contradicted its own prose.** Building an alliance-format tournament surfaced §8.3.1 saying `strict`
...
settle. It is enough to make `strict` look unusable.
...
- Extending a feature and forgetting its checks is its own defect. `OR-905` briefly reported
  ...
  now carry them.
- The alliance-format finding is **recorded and not fixed**: a result belongs to one
...
gap, and the same wall reached twice is that gap reported twice.
