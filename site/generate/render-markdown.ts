...
//
// Names and versions are read from the manifests rather than written into the
// page, because a version in prose is a version that goes stale the next time
// one is released.
...
for (const relative of [
  'sdk/js/packages/core',
  'sdk/js/packages/validate',
  'viewer',
  'validator/cli',
]) {
  const manifest = JSON.parse(await readFile(join(repoRoot, relative, 'package.json'), 'utf8')) as {
    name: string;
    version: string;
    description?: string;
    private?: boolean;
...
await writeFile(
  join(outDir, 'packages.ts'),
  `// Generated by generate/render-markdown.ts. Do not edit.\n` +
    `export interface PublishedPackage {\n` +
    `  name: string; version: string; description: string; path: string;\n` +
    `}\n` +
    `export const packages: PublishedPackage[] = ${JSON.stringify(published, null, 2)};\n`,
  'utf8',
);
...
console.log(
  `Rendered the specification (${specification.toc.length} headings), ` +
    ...
    `${examples.length} examples and ${published.length} published packages.`,
);
