...
**Erratum, 2026-08-10.** The correction above was made in one place and the same defect was left
standing in another. [§5.3.1](#53-attributes) folded an unknown attribute `type` onto `text`
exactly as §5.1.6 folded an unknown `kind`, and [§5.3.4](#53-attributes) then rejected every value
that was not a string — so a 1.1 attribute of type `duration` carrying numbers was reported as
`OR-102` on every entity by a 1.0 consumer. §5.3.1 and §5.3.4 now say that an unrecognised type
implies no value type.

The same reading also reached the reference implementation, which judged every enumeration against
1.0's domains whatever version the document declared, and so rejected six of the seven ways
[§11.2.2](#112-guarantees-to-producers) allows a MINOR to grow — in flat contradiction of
[§11.4.2](#114-version-negotiation). A value this version does not define, in a document declaring
a later MINOR, is now `OR-914`: a warning, because a 1.0 consumer cannot tell such a value from a
typo and refusing the document is the one response §11.4.2 forbids. Nothing caught either defect
...
**§5.3.1** `type` **MUST** be one of `text`, `number`, `date`, `url`, `country`, `boolean`. A
consumer encountering an unknown value **MUST** treat it as `text` **for display**, and **MUST
NOT** infer a value type from it when judging what an entity carries
([§5.3.4](#53-attributes)).
...
**§5.3.4** A value contradicting its declared type is reported as `OR-102`. Where the declared
`type` is one this version does not define, no type is implied and no value contradicts it.
...
| `OR-914` | warning  | An enumeration value this version does not define, in a document declaring a later MINOR (§11.4.2)                                                                               |
...
| [§5.1.5](#51-measures) | precision, when present, MUST be a non-negative integer giving the number of digits to show after the decimal point — not significant figures |
| [§5.1.6](#51-measures) | A consumer encountering an unknown kind MUST treat it as text for display, and MUST NOT infer a value type from it when deciding what a result carries (§8.5.2) |
...
| [§5.2.1](#52-values-and-units) | Every measured quantity MUST be a JSON number, expressed in the unit its measure declares — except for measures of kind text (JSON string) and boolean (JSON boolean) |
...
| [§5.2.5](#52-values-and-units) | A consumer displaying a duration in a time unit — s, ms, min, h — SHOULD render it in hours, minutes and seconds, dropping leading zero components and keeping the declared precision on the seconds |
...
| [§5.3.5](#53-attributes) | An identifier the organisation allocates — a bib, a start number, a car number, a lane — MUST be declared as an attribute, never as a measure |
| [§5.3.6](#53-attributes) | A declared attribute that no entity carries a value for is reported as OR-905, on the same terms and for the same reason as §5.1.7 |
| [§5.3.7](#53-attributes) | An attribute of type number MAY declare a unit, naming what the number counts or measures, and displayed the same way as a measure's |
...
| [§5.4.1](#54-identifiers) | Every producer-assigned identifier — the document id, and those of measures, attributes, participants, events, categories and rankings — MUST match ^[A-Za-z0-9-]+$ |
...
| [§6.1.5](#61-participants) | label names an entity for display, and is REQUIRED on measures, attributes, rankings and categories — the four that a consumer must be able to name on screen |
...
| [§6.1.7](#61-participants) | A declared participant that holds no result and belongs to no team is reported as OR-910, a warning — unless the document carries no results at all, which is an announced event publishing its entry list in advance (§7) |
...
| [§7.2.5](#72-status) | bye marks a competitor who scored without a contest — an odd field in a Swiss pairing, a knockout draw with no opponent, or a walkover |
| [§7.2.6](#72-status) | notClassified marks a competitor who took part, whose performance is recorded, and who does not appear in the classification of the event this result is attached to — short of a minimum distance, short of a qualifying standard, ranked outside a published field |
...
| [§7.5.1](#75-ranks) | ranks, when present, MUST be an object whose keys are declared ranking identifiers and whose values are positive integers |
| [§7.5.2](#75-ranks) | A supplied rank is informative |
| [§7.5.3](#75-ranks) | A key MUST NOT name a ranking that does not rank this result — whether its scope never selected it (§8.5.1) or the partition left it unranked (§8.5.2) |
...
| [§8.3.4](#83-ties) | Under resolved, a group of results comparing equal on every sorting measure MUST be ordered by the positions the producer published for this ranking in ranks (§7.5) — but only when every result in the group carries one and no two of them are equal |
...
| [§10.2.4](#102-extensions) | Any member that is neither defined by this specification nor prefixed x- is a validation error, for the version the document declares |
...
| [§11.3.1](#113-obligations-on-consumers) | A consumer MUST ignore unknown members prefixed x-, and MUST treat unknown enumeration values as the fallback defined for that domain |
...
| [§11.4.1](#114-version-negotiation) | A consumer encountering an unknown MAJOR version MUST NOT attempt to interpret the document, and MUST report the situation distinctly from an invalid document |
| [§11.4.2](#114-version-negotiation) | A consumer encountering a known MAJOR version with a higher MINOR version MUST process the document normally, ignoring what it does not recognise |
...
| [§11.5.3](#115-conformance-levels) | **Consumer, _ranking_ level** — Everything above, plus §8.5 implemented exactly, including sort stability and tie handling |
...
| [§12.1.3](#121-severity) | Every diagnostic MUST carry the location in the document, the rule violated in plain language, and at least one concrete correction |
